...
export function applyThemeColors(
  ...
  if (light) {
    ...
    root.style.setProperty(
      "--color-header-bg",
      light.colorHeaderBg || "#ffffff"
    );
  }

  if (dark) {
    const darkSelector = document.querySelector<HTMLElement>(
      "[data-theme='dark']"
    );
    if (darkSelector) {
      darkSelector.style.setProperty(
        "--color-primary",
        dark.colorPrimary || "#60a5fa"
      );
      darkSelector.style.setProperty(
        "--color-secondary",
        dark.colorSecondary || "#ddd"
      );
      darkSelector.style.setProperty(
        "--color-accent",
        dark.colorAccent || "#93c5fd"
      );
      darkSelector.style.setProperty("--color-bg", dark.colorBg || "#121212");
      darkSelector.style.setProperty(
        "--color-text",
        dark.colorText || "#e5e7eb"
      );
      darkSelector.style.setProperty(
        "--color-header-bg",
        dark.colorHeaderBg || "#121212"
      );
    }
  }
  ...
  document.cookie = `siteSettings=${encodeURIComponent(JSON.stringify(settings))}; path=/; max-age=${60 * 60 * 24 * 365}`;
}
